...
const runConfig = (plugin: any, root: string) =>
...
describe("lankaDiVite — the alias", () => {
  ...
  it("makes a relative root absolute, because a relative alias is a bare specifier", () => {
    ...
    expect(alias).toBe(
      `${resolve("app").replace(/\\/g, "/")}/${lankaDiContract.dirname}`,
    );
  });
  ...
  it("lets an explicit root override what vite resolved", () => {
    ...
    const config = runConfig(
      lankaDiVite({ root: mine }),
      "C:/somewhere/else",
    ) as {
      resolve: { alias: Record<string, string> };
    };

    expect(config.resolve.alias[lankaDiContract.alias]).toContain(
      mine.replace(/\\/g, "/"),
    );
  });
});
...
describe("lankaDiVite — buildStart", () => {
  it("scaffolds a fresh consumer and warns, so the files get committed", () => {
    ...
    expect(existsSync(join(root, lankaDiContract.dirname, "Host.ts"))).toBe(
      true,
    );
  ...
  it("refuses to generate anything when scaffolding is off, as in CI", () => {
    ...
    expect(() =>
      runBuildStart(lankaDiVite({ root, scaffold: false }), ctx),
    ).toThrow(new RegExp(`${lankaDiContract.dirname}/ is missing`));
