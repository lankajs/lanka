...
describe("only lanka — the inputs live in the ViewModel", () => {
	// The scene that RENDERS this pair — one input re-rendering and its
	// neighbour not — lives in `@lankajs/react`'s playground, because what it
	// proves is a binding's work. What is proved here is the shape underneath
	// it: flat keys rather than one `values` object, so the two are separable at
	// all.
