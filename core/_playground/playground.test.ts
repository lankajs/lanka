...
describe("the playground application", () => {
	it("boots, fetches, and the state carries what the server answered", async () => {
		app = await startPlayground(createPlaygroundTransport(todos()));

		await app.useTodosVM.getState().load();

		expect(app.useTodosVM.getState().todos.map((one) => one.title)).toEqual([
			"write the canon",
			"run the canon",
		]);
	});
	...
	it("names the failure the framework named, not a raw throw", async () => {
		app = await startPlayground(createPlaygroundTransport(todos()));
		...
		// fallback for when there is none — and no status code reaches the state.
		expect(app.useTodosVM.getState().error).toBe("no such todo");
	});
