...
export const PACKAGES = [
	// ─────────────────────────────────────────────────────────── CORE ──
	{
		...
		short: "lanka",
		title: "Core",
		gist: "Fourteen subsystems, one peer dependency, six extension points.",
	...
	},
	{
		...
		runtime: ["browser", "node", "native"],
		hasTests: true,
		devDeps: { "@lankajs/tool-testing": "workspace:^", typebox: "^1.3.34" },
		deps: { lanka: "workspace:^" },
		// TypeBox 1.x, published under a new name with a new API. 0.34 is what the 1.x line
		// of this package binds; see the note below.
		peer: { typebox: "^1.3.34" },
		contains: [
	...
	},
	{
		...
		devDeps: {
			...
			// The TypeBox the playground builds its billing schema with — the one
			// @lankajs/typebox binds.
			typebox: "^1.3.34",
	...
	},
	{
		...
		slug: "relay",
		short: "relay",
		title: "Scenarios between applications that cannot share one lanka",
		gist: "Two applications on one page, each with its own copy of the framework, hear each other's scenarios.",
		runtime: ["browser"],
		hasTests: true,
		contains: [
		...
		devDeps: {
			"@lankajs/tool-testing": "workspace:^",
			// The PUBLISHED 0.1.0, under an alias: the page protocol is held to the
			// copies that already shipped, not to a fixture written to resemble them.
			"@lankajs/plugin-relay-0.1.0": "npm:@lankajs/plugin-relay@0.1.0",
			// Bundles the package's own source for a worker thread — the second realm
			// `lankaRelay.realms.test.ts` runs in.
			esbuild: "^0.28.2",
		},
		socket: "use(plugin) · lankaEventBus.addObserver",
		notes: [
	...
	},
	{
		...
		gist: "One command that wires lanka into a project: a template, the packages it needs, the barrels, the build config and a feature written in the layers.",
		runtime: ["node"],
		hasTests: true,
		bin: { "lanka-init": "cli" },
		entries: ["cli"],
		// The contract, at runtime, from the package that owns it. The barrel file
		// names and the export the framework reads by name are `@lankajs/tool-di`'s
		// declaration; a copy here would be a second answer to "what is a barrel",
		// and the two would part company on the day the contract version moves.
		deps: { "@lankajs/tool-di": "workspace:^" },
		contains: [
