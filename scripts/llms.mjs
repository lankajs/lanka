...
export const renderLlmsIndex = () => {
	...
	const lines = [
		"# lanka",
		"",
		"> A layered application framework, bound to no UI library: gateways for",
		"> transport, ViewModels for what a screen holds, scenarios for what happens",
		"> between screens. Imports go one way and a lint rule says so.",
		`> Core version ${version}; ${String(PACKAGES.length)} packages, versioned`,
		"> independently — each document names the version it describes.",
		"",
		"Install the core with `npm install lanka zustand` — `zustand` is core's one peer",
		"dependency, and only npm adds a missing peer for you. Reading a ViewModel from a",
		"screen is one package more, the binding for your UI framework. Everything else is",
		"optional and installed one package at a time.",
		"",
		...
		`- [COMPATIBILITY.md](${rawUrl("COMPATIBILITY.md")}): which package runs where, with which UI framework and peers, and what holds with several applications or copies on one page`,
