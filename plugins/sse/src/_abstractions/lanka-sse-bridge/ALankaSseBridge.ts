...
import type { ILankaSseTriggerContext } from "../../_factories/create-lanka-sse-trigger-context/createLankaSseTriggerContext";
 ...
 */
export abstract class ALankaSseBridge {
	private readonly unsubscribes: (() => void)[] = [];

	protected readonly sse: ILankaServerEventTransport;
	protected readonly trigger: ILankaSseTriggerContext;
	...
	public constructor(sse: ILankaServerEventTransport, trigger: ILankaSseTriggerContext) {
		this.sse = sse;
	 ...
	 */
	protected on(eventType: string, handler: TLankaSseEventCallback): void {
		this.unsubscribes.push(
			this.sse.on(eventType, (data) => {
				this.trigger.run(() => handler(data));
			}),
		);
	}
