/**
 * `@lankajs/angular/testing` — rendering a Angular tree with a bootstrapped framework.
 *
 * A separate entry because the library it needs is one a consumer should not be
 * asked to install in order to ship an application: `@testing-library/angular` is an
