 ...
 * and it says nothing about React: `lanka/viewmodel` is core, and `build()`
 ...
 * what the CALL answers — here a plain state, because this is React.
 *
 * At MODULE level, like both siblings: the wrapper is `useLankaVM` pre-applied
 * and not a hook body, so nothing is subscribed and nothing is rendered until a
 * screen calls the result. `createActions` writes the class's name into
