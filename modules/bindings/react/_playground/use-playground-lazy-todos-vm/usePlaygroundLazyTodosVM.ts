 ...
 * makes.
 *
 * `toLankaReactVM` forwards through a Proxy rather than copying members, so
 * wrapping a lazy ViewModel does not spend its laziness: this module is imported
 * with every scene in the playground and builds nothing until a screen reads it.
 * A wrapper that copied `name`, `getState` and `subscribe` onto a new object
