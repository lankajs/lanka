 ...
 * and it says nothing about Svelte: `lanka/viewmodel` is core, and `build()`
 ...
 * what the CALL answers — here an object of getters, because this is Svelte.
 *
 * A plain `.ts` module and not `.svelte.ts`, for the reason its siblings are:
 * no rune is used here, because wrapping is not a read. It runs at IMPORT,
 * outside every effect, and `createActions` writes the class's name into
 * `playgroundVMBuildLog` — so a scene can see that this one, unlike the lazy
 * neighbour, built its store there and then.
 */
