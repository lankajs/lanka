...
export const createPlaygroundForm = () => {
	...
	return {
		...
		submit(input: unknown): boolean {
			const result = lankaTypeBoxValidator.validateSafe(playgroundSignUpSchema, input);
		...
		parseOrThrow: (input: unknown): TPlaygroundSignUp =>
			lankaTypeBoxValidator.validate(playgroundSignUpSchema, input, "playground sign-up"),
	};
};
