...
export const createPlaygroundForm = () => {
	...
	return {
		...
		submit(input: unknown): boolean {
			const result = lankaEffectValidator.validateSafe(playgroundSignUpSchema, input);
		...
		parseOrThrow: (input: unknown): TPlaygroundSignUp =>
			lankaEffectValidator.validate(playgroundSignUpSchema, input, "playground sign-up"),
	};
};
