...
/**
 * The package, used as a form uses it — and asked the same questions as the zod
 * playground, because the two are promised to be interchangeable.
 */
const valid = { email: "ada@example.com", age: 36, tags: [{ id: 1 }] };

describe("the valibot playground", () => {
...
describe("a backend that speaks another shape", () => {
	it("maps the wire into the application's vocabulary in one call", () => {
		const domain = lankaValibotValidator.validate(
	...
	it("maps back into the payload that backend expects", () => {
		const payload = lankaValibotValidator.validate(
	...
	it("refuses a wire the mapping cannot read, naming which contract broke", () => {
		const result = lankaValibotValidator.validateSafe(playgroundApiShapeSchema, {
