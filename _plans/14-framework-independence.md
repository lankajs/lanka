...
Осталось: `_playgrounds/solid/solid-start` (шестой хост; `spa` закрывает
экосистему, Vinxi/Nitro — нет), `atlasScenes.ts` + `hosts.mjs` + гейт
`check-playgrounds`, фаза 14.8, и замеры `check:perf` на простаивающей машине —
включая ПОВТОРНЫЕ для трекера, чья запись изменилась после того, как базлайн был
снят. Замеры сознательно НЕ сняты здесь: машина была занята сборками весь день, а
