...
**Статус:** 14.0–14.7 сделаны и закоммичены, плюс перекладка `_playgrounds/` по
экосистемам, `react/_shared`, `_playgrounds/vanilla` целиком и фасад
`toLankaReactVM`. Осталось: приложения для четырёх новых
фреймворков, острова Astro, `atlasScenes.ts`/`hosts.mjs` и гейт
`check-playgrounds`, фаза 14.8, и четыре новых базлайна трекера
(`check-perf --write` на простаивающей машине).
