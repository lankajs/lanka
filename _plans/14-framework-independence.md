...
**`solid-start` заблокирован мажором vite — но блокер сменился, и это
проверено 2026-09-19.**
`@solidjs/start@2.0.5` требует `vite: ^8 || ^9`; репозиторий на `^7.3.1`.
...
Экосистемной причины больше нет: vite 8.3.0 вышел, и вся цепочка его уже
принимает — `vitest@4` (`^6 || ^7 || ^8`), `@vitejs/plugin-react` (`^8`),
`@vitejs/plugin-vue`, `vite-plugin-solid`, `@sveltejs/vite-plugin-svelte@8`,
`@analogjs/vite-plugin-angular`, `@sveltejs/kit`. Проверено `npm view` по
каждому, а не предположено.

Осталась цена, и она не в solid-start: поднять vite 7→8 значит поднять
vitest 3→4 во всех 58 проектах — другой мажор тест-раннера, с другими
умолчаниями coverage, другой формой `projects` и другим поведением `vi.mock`.
Это отдельная миграция со своим планом, а не шаг этого, и записывается здесь
именно так: не «невозможно», а «стоит мажора раннера». Претензия, которую
шестой хост должен был доказать — что раздел § Nitro в `modules/host/GUIDE.md`
общий, а не написан под Nuxt, — остаётся доказанной ОДИН раз, и это долг.

**План 14 закрыт.** 14.8 сделана 2026-09-19 в тир `lanka/extend`: проход
`surface-architect` нашёл в первой редакции два дефекта — оба подтверждены по
исходникам и оба закрыты швом области в ядре и швом `buildScoped`. На фасаде не
появилось ни одного имени, поэтому необратимого обещания не дано; перегрузка в
пяти привязках осталась отдельным решением для плана 15.

Замеры `check:perf` сняты 2026-09-19: гейт зелёный, 70 операций в 16 пакетах.
Четырнадцать из пятнадцати расхождений оказались не регрессиями — бенчи ядра
гонялись в ДВУХ окружениях, а базлайн ключуется по файлу и имени, так что
побеждало то, что закончило последним. Пятнадцатое было настоящим и исправлено.
...
**Механизм сделан 2026-09-19, в тир `lanka/extend`. Фасад не тронут.**

Форма была спроектирована, отправлена на отдельный проход `surface-architect`,
как этот раздел и требовал, и проход нашёл два дефекта. Оба подтверждены по
исходникам и оба закрыты — ниже написано и то, и другое, потому что механизм в
первой редакции делал РОВНО ТО, что фаза должна запретить.

**Дефект 1 — вьюмодель области с обработчиками сценариев протекала во все
следующие запросы.** `ALankaVM.build()` зовёт `registerViewModel`, тот пишет в
`declaredViewModels` — приватный массив уровня ПРОЦЕССА
(`LankaScenarioBootstrap.ts`), который очищается только через
`reset({ withDeclarations: true })`. А `createLanka` зовёт
`adoptDeclaredViewModels()` на КАЖДОМ экземпляре. Одна на модуль — верно и
несуще; одна на ЗАПРОС — и запрос N+1 усыновляет вьюмодель запроса N, подписывает
её на свои сценарии, после чего диспатч выполняет обработчики N против шлюзов N и
пишет в стор N. Все сюиты при этом зелёные.

Закрыто швом `lankaScenarioBootstrap.buildScoped(fn)`: объявления внутри него
идут в активный экземпляр и не попадают в процессный список. Узкий шов, а не
параметр на девяти фабриках — восьми из них незачем знать, что такое область.
Сцена доказана обоими концами: она падает, если снять правку, и проходит с ней.

**Дефект 2 — `resolveLankaVM` не падал вне области там, где это важно.**
`createLanka` зовёт `activate()` безусловно, а `runInLankaServerScope` зовёт
`createLanka` ВНУТРИ `AsyncLocalStorage.run` — значит во время запроса указатель
процесса и рантайм области это один и тот же объект, и «внутри запроса» от «после
него» неотличимо. Резолвер рантайма вне области отдаёт процессный, так что вызов
после запроса разрешался в рантайм последнего постороннего.

`requireActiveRuntime` — неверный примитив: он отвечает «какой ЭКЗЕМПЛЯР», а фазе
нужно «какая ОБЛАСТЬ». Поэтому в ядре появился второй шов —
`setLankaScopeResolver` / `getActiveLankaScope` / `hasLankaScopeResolver`
(`lanka/internal`), а `@lankajs/host` ставит его из своего `AsyncLocalStorage`
рядом с резолвером рантайма. Установлен резолвер и ответил `null` — значит код
вне запроса, и это падает по имени определения. Запасного пути нет намеренно:
неверный ответ здесь — это данные одного пользователя на странице другого.

**Что ещё из ревью учтено.** `ILankaVMDefinition` НЕПРОЗРАЧЕН — бренд на
неэкспортируемом символе, рецепт в приватном `WeakMap`; иначе тип стал бы портом
в день публикации, и §6b запретил бы добавлять в него что-либо навсегда. Обе
карты слабые с обоих концов. Второго имени у одной строки нет. Заголовок
открывается тем, когда НЕ надо. Перегрузки в пяти привязках нет — это отдельное
решение, а `useLankaVM(resolveLankaVM(vm))` работает сегодня на всех пяти ценой
одной пары скобок.

**Почему `extend`, а не фасад.** Форма молодая: браузеру она не нужна, серверу
нужна вся, а вопрос про перегрузку не решён. `lanka/extend` меняется в миноре —
это ровно тот запас, который такому решению нужен. На фасаде не появилось ни
одного имени, то есть необратимого обещания не дано.

**Осталось для плана 15:** перегрузка `useLankaVM(definition)` в пяти привязках с
сценой 12 в сюите соответствия и идиомой в каждой (`skills/parity/SKILL.md` §3c,
правило 5 — всё одним коммитом); переезд формы из `next/README.md` с `useState`
на scoped VM в четырёх хостах; и выпуск на фасад, когда перегрузка решена.
