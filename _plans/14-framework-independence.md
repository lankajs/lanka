...
**Статус на 2026-09-19.** Закрыты 14.0–14.7 ПОЛНОСТЬЮ: пять привязок на полке,
и теперь по приложению на каждую экосистему. Сверх плана закрыто то, что
появилось по ходу: перекладка `_playgrounds/` по экосистемам, `react/_shared`,
`_playgrounds/vanilla` и `_playgrounds/node` целиком, идиома в каждой из пяти
привязок с правилом в каноне и гейтом, контракт для чужих привязок, и все девять
форм вьюмодели в сюите и в каждом плейграунде.

Приложения, закрытые 2026-09-18/19:

| Экосистема | Пакеты                              | Что нашло                                                                                     |
| ---------- | ----------------------------------- | --------------------------------------------------------------------------------------------- |
| `svelte`   | `_shared`, `spa`, `sveltekit`       | `svelte-check` вместо `tsc`; область НЕ открывается в `hooks.server.ts`; VM на рендер, наоборот к Nuxt |
| `solid`    | `_shared`, `spa`                    | сюита без единого flush; `<Show>`/`<For>` не стиль; `use` без правил хуков — и гейт линта сузился |
| `angular`  | `_shared`, `spa` (SPA + SSR в одном)| VM — это ПРОВАЙДЕР, а не `input`: `input.required` не читается в инициализаторе поля (NG0950)  |
| `astro`    | четыре острова на одной странице    | два JSX-диалекта в одном проекте; делегирование событий в Solid и Svelte 5                     |

`modules/host/GUIDE.md` получил разделы Nuxt, SvelteKit и Angular — три новых
хоста, ни одного нового пакета.

`atlasScenes.ts` + `hosts.mjs` + гейт `check-playgrounds` сделаны 2026-09-19 —
тринадцатый гейт в цепочке, и на первом честном прогоне он нашёл шестнадцать
расхождений, включая кнопку сортировки в React-приложении, на которую не было
сцены с самого начала, и полное отсутствие `runLankaStatic` у Angular.

**`solid-start` заблокирован мажором vite, и это не обходится.**
`@solidjs/start@2.0.5` требует `vite: ^8 || ^9`; репозиторий на 7.3.6.
`@solidjs/start@1.3.2` требует `vinxi ^0.5.7`, который приносит свой vite 6 —
то есть второй мажор vite в графе, против которого `lankaDiVite` типизирован.
Поднимать весь монорепозиторий до vite 8 ради шестого хоста — это отдельная
миграция со своим планом, а не шаг этого. Претензия, которую он должен был
доказать — что раздел § Nitro в `modules/host/GUIDE.md` общий, а не написан под
Nuxt, — остаётся доказанной ОДИН раз, и это записано здесь как долг.

**План 14 закрыт на 14.7.** Фаза 14.8 разобрана ниже и вынесена в план 15:
проход `surface-architect` нашёл в её форме два дефекта, из-за которых механизм
делает ровно то, что фаза должна запретить, и оба подтверждены по исходникам.
Сам раздел сохранил это как материал.

Осталось из 14: замеры `check:perf` на простаивающей машине — включая
ПОВТОРНЫЕ для трекера, чья запись изменилась после того, как базлайн был снят.
Замеры сознательно НЕ сняты здесь: машина была занята сборками весь день, а
`skills/gates/SKILL.md` §1 говорит, почему число с занятой машины хуже, чем его
отсутствие.
...
**`_playgrounds/vue/nuxt` сделан 2026-09-15.** HOST-контракт: `server/api/missions.get.ts`
→ `runLankaRequest` с заголовками Nitro; `runLankaStatic` для пререндера;
payload → `hydrateLankaVM` в компоненте; `lankaDiVite` внутри `vite.plugins`
Nuxt — потому что сборка Nuxt ЕСТЬ Vite, и это тот же плагин, что у SPA.

Найденная ловушка, у которой нет реактовского аналога: **тело `<script setup>` —
это `setup()`**. Оно выполняется на каждый ЭКЗЕМПЛЯР компонента, значит
вьюмодель, объявленная там, — новый стор на каждый монтаж, и `hydrateLankaVM`,
применяющийся один раз на стор, применяется к каждому из них. Второй рендер с
другими данными заменил строки первого — ровно то, что гидратация обязана
отвергать, — и экран выглядел правильно, пока их не стало двое. Вьюмодель живёт
в обычном блоке `<script>`, и сцена следит, чтобы она там осталась.

Мелочи, которые стоило записать: `NUXT_ATLAS_API` через `process.env`, а не
`import.meta.env` — файл читают обе стороны; `nuxt prepare` перед тайпчеком,
потому что Nuxt ГЕНЕРИРУЕТ типы своих авто-импортов; наши собственные файлы
импортируют по имени (`useFetch` из `nuxt/app`, `defineEventHandler` из `h3`,
`defineNuxtConfig` из `nuxt/config`) — имя без строки импорта читатель не
проследит, а тайпчек тогда зависел бы от сгенерированного.

14 сцен, 100/100/100/100, ратчет 99/99/99/99.
...
**Сделано 2026-09-18.** Привязка 2026-09-15, оба приложения 2026-09-18.
`_playgrounds/svelte/{_shared,spa,sveltekit}` в репозитории, зелёные, с
измеренными храповиками. `createSubscriber` вместо
...
**Сделано частично 2026-09-19.** Привязка 2026-09-15,
`_playgrounds/solid/{_shared,spa}` 2026-09-19. `solid-start` НЕ сделан — это
шестой ХОСТ, и раздел § Nitro из 14.4 всё ещё ждёт проверки тем, что подойдёт
без правок. Сигнал с `equals: false`:
...
**Сделано 2026-09-19.** Привязка 2026-09-15, приложение 2026-09-19:
`_playgrounds/angular/{_shared,spa}`, где `spa` — это SPA И серверный рендер в
ОДНОМ проекте, потому что у Angular это второй вызов, а не второй проект.
...
**НЕ делается в плане 14. Это план 15, и вот почему — с номерами строк.**

Форма была спроектирована и отправлена на отдельный проход `surface-architect`,
как этот раздел и требовал. Проход нашёл два дефекта, и оба подтверждены по
исходникам: механизм в предложенном виде делает РОВНО ТО, что фаза должна
запретить.

**Дефект 1 — вьюмодель области с `scenarioHandlers` протекает во все следующие
запросы.** `ALankaVM.build()` зовёт `registerViewModel`, тот пишет в
`declaredViewModels` — приватный массив уровня ПРОЦЕССА
(`LankaScenarioBootstrap.ts:178`), который очищается только через
`reset({ withDeclarations: true })` (`:279`). А `createLanka` зовёт
`adoptDeclaredViewModels()` на КАЖДОМ экземпляре
(`createLanka.ts:292`). Сегодня это верно и несуще: вьюмодель одна на модуль.
Под 14.8 вьюмодель одна на ЗАПРОС, поэтому массив растёт на запись за запрос за
всю жизнь процесса, а запрос N+1 усыновляет вьюмодель запроса N и подписывает её
на свои сценарии. Сценарий, отправленный в N+1, выполняет обработчики N против
шлюзов N и пишет в стор N — то есть межзапросная течь приходит через дверь, за
которой никто не смотрел. `lanka.dispose()` до `declaredViewModels` не достаёт.

**Дефект 2 — `resolveLankaVM` НЕ падает вне области там, где это важно.**
`createLanka` зовёт `instance.activate()` безусловно (`createLanka.ts:291`), а
`activate()` — это `setActiveLankaRuntime(instance)` (`:234`); а
`runInLankaServerScope` зовёт `createLanka` ВНУТРИ `AsyncLocalStorage.run`.
Значит указатель уровня процесса становится рантаймом текущего запроса. И
`lankaServerRuntimeResolver` вне области ОТДАЁТ его:
`scope ? scope.runtime : getLankaProcessRuntime()`. Итог: вызов вне всякой
области на сервере разрешается в рантайм последнего запроса и отдаёт — или
СТРОИТ И КЭШИРУЕТ — вьюмодель последнего постороннего. Это исходный провал «one
per PROCESS» в свежей краске, и теперь он держит пользовательское состояние
намеренно.

`requireActiveRuntime` — неверный примитив для этой фазы. Он отвечает «какой
экземпляр фреймворка», а фазе нужно «какая ОБЛАСТЬ», и этого ядро не моделирует.
Падать вне области — правильное поведение, и запасного пути быть не должно
(рассуждение в `activeRuntime.ts:84-98` здесь сильнее, чем где-либо: неверный
ответ тут — это данные пользователя, а не не та шина). Но кода, который так
делает, нет.

**Что план 15 должен решить до первого имени в бочке:**

1. **Сцена области в ядре.** Либо внутренний шов `getActiveLankaScopeId(): object | null`, который заполняет `AsyncLocalStorage` хоста, и ключ по ОБЛАСТИ, а не по рантайму; либо отказ, когда резолвер установлен и ответил рантаймом процесса. Без одного из двух заголовок «падает вне области» — ложь в докблоке, а ложную причину процитируют во время инцидента.
2. **Снятие с учёта при `dispose`.** Экземпляр области обязан уходить из `declaredViewModels`, иначе дефект 1 неисправим изнутри фабрики. Это требует хука, которого у модульного `WeakMap` нет — значит поле на `ILankaRuntime` (тир `internal`, `modules/host` его уже импортирует) или доступ для `dispose()`.
3. **Непрозрачное определение.** `ILankaVMDefinition` с публичным `build` делает тип ПОРТОМ: потребитель напишет его руками, вызовет `build()` мимо карты, и §6b запретит добавлять в него что-либо навсегда. Бренд-поле плюс приватный `WeakMap` с конфигом стоит одну строку и возвращает всё пространство решений, включая вызываемую форму `missionsVM()`.
4. **Внутренняя карта — тоже `WeakMap`.** Публичный тип приглашает `defineLankaVM({...})` внутри компонента или цикла; в браузере рантайм живёт сколько вкладка, и каждое такое определение — вечная запись плюс вечная вьюмодель.
5. **Перегрузка в пяти привязках — ОТДЕЛЬНОЕ решение и последнее.** `useLankaVM(resolveLankaVM(vm))` работает сегодня на всех пяти без единой правки фреймворка: разрешённый экземпляр стабилен внутри области, так что зависимость `useCallback` в React не двигается. Перегрузка стоит десять постоянных мест (пять фасадов плюс пять идиом по `skills/parity/SKILL.md` §3c, и все в одном коммите по правилу 5), а покупает одну скобку.
6. **Два имени у одной строки.** `name` есть и на определении, и на построенной вьюмодели (`ILankaVMConfig.name`), и ничто их не сверяет — а диагностика, предупреждение о слепом пятне и реестр сценариев читают второе. Врать может первое.
7. **Чем это становится.** Имя `defineLankaVM` имеет прецедент (`defineLankaRole`, `defineLankaPrefetchResource`, `defineStore`), и глагол свою работу делает. Риск не в синонимии, а в ПРЕДШЕСТВОВАНИИ: пришедший из Pinia прочтёт его как новый способ по умолчанию и завернёт в него все девять фабрик, включая браузерные приложения, где это покупает ноль. Заголовок обязан открываться тем, когда НЕ надо.

Черновик `core/src/viewmodel/_factories/define-lanka-vm/` удалён, а не оставлен
под флагом: незапущенный файл в `_factories/` читается как работа в полёте, и
следующий человек потратит час, выясняя, почему у него нет теста.

Исходная формулировка фазы — ниже, как материал для плана 15.
...
что именно они доказали. Это условие выполнено; невыполненными оказались два
других, и они выше.
