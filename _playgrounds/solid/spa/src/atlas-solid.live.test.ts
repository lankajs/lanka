...
/**
 * The Solid application's start-up, against the real API.
 *
 ...
 *
 * Nothing here renders, which is the point: start-up is the half of a Solid
 * application that has no Solid in it, and this file is where that is checked
...
let base: string;
let solid: IAtlasSolidApp | null = null;
...
const start = async (connect = false): Promise<IAtlasSolidApp> => {
	solid = await startAtlasSolid({ apiBaseUrl: base, connect });

	return solid;
};

describe("starting a Solid application against the real API", () => {
	...
	it("reads the board over the real wire, with no renderer anywhere", async () => {
		...
		// The same gateway the React, Vue and Svelte applications call, from a file that
