...
OWN, by Vite or by webpack, loaded onto one page by a shell. Some share the
...
[`buildWithWebpack`](./src/Core/Build/buildWithWebpack.ts) the two bundlers,
and [`isProvidedByThePage`](./src/Core/Build/isProvidedByThePage.ts) the one
rule both apply — so a Vite bundle and a webpack bundle of the same module
differ in the bundler and nothing else.

Twelve bundles: every framework through both bundlers, and each bundler
...
every framework goes through both bundlers, and each bundler produces both a
module that shares lanka and one that carries its own. A bundle built and never
loaded would look covered and not be.

## The scenes

**[`oneLanka.test.ts`](./src/oneLanka.test.ts)** — modules from both bundlers
over the shell's one lanka. The shell's scenario reaches all four frameworks; a
write that starts in one module — the React module's button, through its own
ViewModel's action — repaints the other three; and every module leaves with its
ViewModels when the shell closes the scope it handed over. Make webpack bundle
lanka into the modules that should share it and all three fail.
...
four frameworks, two bundlers, three copies of lanka, one fact. An app that
loads after the one that assigned has LEFT is still handed the value, because
the shell retained it. Take the shell's relay away and all three fail.
...
- **Angular** is compiled in the page (JIT) and declared by calling
  ...
  through Vite and webpack alike without an Angular plugin in either. Zoneless;
...
- **Svelte** is the one module a bundler must be told about — Vite through its
  Svelte plugin, webpack through `svelte-loader` — and the same `.svelte` file
  goes through both.
- **React and Vue** need nothing beyond TypeScript: esbuild inside webpack,
  Vite's own for Vite. The Vue module is a render function, so this package
...
- **Rollup builds one module per `vite build`**, since it would hoist what two
  entries share into a chunk both load — one copy of lanka. Webpack builds a
  pipeline in one compilation: with `splitChunks` off, each entry file carries
...
About half a minute of the run is the twelve builds. No dev server: there is no
