 ...
 * needs it for the same reason: `{{ code }} {{ title }}` renders as separate
 * text nodes, which looks identical on screen and means a reader — a test, a
 * screen reader — cannot match the line.
 *
 * Text and not a component, so it works wherever Vue does.
 */
