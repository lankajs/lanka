...
import { AtlasBoardIslandSolid } from "./AtlasBoardIslandSolid";
...
describe("AtlasBoardIslandSolid", () => {
	it("starts from what the page already fetched", () => {
		...
		render(() => <AtlasBoardIslandSolid missions={FROM_THE_PAGE} />);
	...
	it("is interactive, which is the whole reason it is an island", () => {
		render(() => <AtlasBoardIslandSolid missions={FROM_THE_PAGE} />);
		const input = screen.getByLabelText<HTMLInputElement>("Search Solid missions");
	...
	it("hydrates once, so a second render cannot replace what a person is looking at", () => {
		...
		render(() => <AtlasBoardIslandSolid missions={[mission("m-9", "Something else")]} />);
