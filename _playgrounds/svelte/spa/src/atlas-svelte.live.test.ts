...
/**
 * The Svelte application's start-up, against the real API.
 *
 ...
 *
 * Nothing here renders, which is the point: start-up is the half of a Svelte
 * application that has no Svelte in it, and this file is where that is checked
...
let base: string;
let svelte: IAtlasSvelteApp | null = null;
...
const start = async (connect = false): Promise<IAtlasSvelteApp> => {
	svelte = await startAtlasSvelte({ apiBaseUrl: base, connect });

	return svelte;
};

describe("starting a Svelte application against the real API", () => {
	...
	it("reads the board over the real wire, with no renderer anywhere", async () => {
		...
		// The same gateway the React and Vue applications call, from a file that
