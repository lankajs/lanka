 ...
 * as `any` and the `??` below it looks redundant to a reader and to a linter.
 *
 * `svelte` and not a `declare module "*.svelte"` shim, which Vue's application
 * does need: this project is typechecked by `svelte-check`, which reads
 * components properly, and a shim would REPLACE that knowledge with
