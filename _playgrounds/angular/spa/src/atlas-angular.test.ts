...
import type { TAtlasMissionsVM } from "@lanka-playgrounds/angular-shared";
...
describe("the seams between a binding and the ViewModel it reads", () => {
	it("counts through a SELECTOR, and keeps counting when the board is filtered away", async () => {
		...
		const queued = (): string =>
			fixture.nativeElement
				.querySelector("[data-testid=queued-count]")
				?.textContent?.trim() ?? "";

		expect(queued()).toBe("2 queued");
	...
	it("lets go of the ViewModel when the screen is taken away", async () => {
		...
		// reports it.
		//
		// Angular is the one ecosystem where the release is not the framework's
		// doing: `useLankaVM` registers with `DestroyRef`, which is why it must run
		// in an injection context, and this scene is what proves that registration
		// happened rather than merely compiled.
		const { live, viewModel } = watched(createAtlasMissionsVM(fakeMissionGateway()));
		TestBed.configureTestingModule({
			providers: [
				provideZonelessChangeDetection(),
				{ provide: ATLAS_MISSIONS_VM, useValue: viewModel },
				{ provide: ATLAS_BOARD_VM, useValue: new AtlasBoardVM(fakeBoardGateway()).build() },
				{ provide: ATLAS_AVATARS, useValue: avatars() },
			],
		});
		const fixture = TestBed.createComponent(AtlasMissionsScreen);
		fixture.detectChanges();
	...
	it("moves two screens that read one ViewModel", async () => {
		...
		const { missionsVM } = configure(fakeMissionGateway());
		const first = TestBed.createComponent(AtlasMissionsScreen);
		const second = TestBed.createComponent(AtlasMissionsScreen);
		first.detectChanges();
		...
		expect(text(first)).toContain("Restock the depot");
		expect(text(second)).toContain("Restock the depot");

		missionsVM.getState().applySearch("ridge");
		first.detectChanges();
		second.detectChanges();

		expect(text(first)).not.toContain("Restock the depot");
		expect(text(second)).not.toContain("Restock the depot");
		expect(text(second)).toContain("Survey the north ridge");
	});
});
