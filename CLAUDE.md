...
| which package runs where, needs what           | `COMPATIBILITY.md` — generated by `scripts/compatibility.mjs`           |
...
7. **Generated files are edited through their generator.** `README.md`,
   ...
   `COMPATIBILITY.md`, `llms.txt`, `api/*.api.md`, `perf/*.perf.md` and every
   package's `LICENSE` — edit the registry, the script or the root `LICENSE`,
   then regenerate. `check:drift` refuses a hand edit.
